...
const animeList = computed<AnimeItem[]>(() => {
  return baseAnimeList.value.map((animeItem) => {
    const animeUserStatus = animeUserStatuses.value.find((animeUserStatus) => {
      return animeUserStatus.anime_id === animeItem.id;
    });

    return {
      ...animeItem,
      status: animeUserStatus
        ? animeStatuses.value.find((animeStatus) => {
          return animeStatus.id === animeUserStatus.status_id;
        })
        : undefined
    };
  });
...
const animeListFiltered = computed<AnimeItem[]>(() => {
  ...
  if (filter.name) {
    filteredData = filteredData.filter((animeItem) => {
      return animeItem.name.toLowerCase().replace('ё', 'е')
        .includes(filter.name.toLowerCase().replace('ё', 'е'));
    });
  }
...
<template>
  <ClientOnly>
    <UContainer class="py-4 space-y-4">
      ...
      <UCard
        :ui="{
          background: 'bg-white/75 dark:bg-gray-900/75',
          base: 'sticky top-[72px] z-50 -mx-2 backdrop-blur',
          body: { base: 'flex gap-4' }
        }"
      >
        <UButton
          icon="i-heroicons-plus-20-solid"
          @click="modalAnimeFormIsOpen = true"
        />
        <div
          class="flex-1 grid sm:grid-cols-1"
          style="gap:inherit"
        >
          <UInput
            v-model="filter.name"
          ...
          >
            ...
            >
              <template #label>
                <span
                  v-if="filter.statuses.length"
                  class="truncate"
                >{{ filter.statuses.map((item) => item.value).join(', ') }}</span>
                <span
                  v-else
                  class="text-gray-400 dark:text-gray-500"
                >Фильтр статусов...</span>
              </template>
      ...
      >
        ...
        >
          <UCard
            :ui="{
              base: [
                'relative z-0 flex flex-col justify-between w-full h-full',
                'text-gray-900 dark:text-white pointer-events-none'
              ],
              background: [
                'has-[a:hover]:bg-gray-100/50 dark:has-[a:hover]:bg-gray-800/50'
              ],
              divide: 'divide-y-0',
              ring: [
                'ring-1 has-[a:hover]:ring-2 ring-gray-200',
                'has-[a:hover]:ring-primary-500 dark:ring-gray-800',
                'dark:has-[a:hover]:ring-primary-400'
              ],
              body: { base: 'flex justify-between items-end gap-x-2' },
              header: {
                base: 'flex justify-between items-start gap-x-2',
                padding: 'pb-0'
              }
            }"
          >
            <template #header>
              <h2 class="text-lg/tight font-semibold line-clamp-2">
                <ULink
                  :class="[
                    'focus-visible:outline-none pointer-events-auto before:absolute',
            ...
            <div>
              <Paragraph
                v-if="animeItem.date_release"
                :label="labels.date_release"
                :value="dateFormat(animeItem.date_release)"
              />
              <Paragraph
                v-if="animeItem.date_finish"
      ...
      <ModalAnimeForm
        v-model="modalAnimeFormIsOpen"
        @success="getAnimeList"
      />
    </UContainer>
  </ClientOnly>
</template>